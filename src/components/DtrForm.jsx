    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-soft p-5 sm:p-6 md:p-8 w-full transition-colors mb-8 min-w-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 border-b border-slate-100 dark:border-slate-800 pb-4 min-w-0">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="bg-cyan-500/10 p-2.5 rounded-xl text-cyan-600 dark:text-cyan-400 border border-cyan-500/25 flex-shrink-0">
            <PlusCircle className="w-5 h-5" />
          </div>
          <div className="min-w-0">
            <h3 className="text-base font-black text-slate-900 dark:text-white truncate">Log New Hours</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 truncate">Record your daily OJT hours below.</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4 sm:gap-5 items-end min-w-0">
        <div className="w-full min-w-0">
          ...
          <input type="date" required value={date} onChange={(e) => setDate(e.target.value)} 
            className="w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-cyan-500 transition-all min-w-0" />
        </div>
        <div className="w-full min-w-0">
          <div className="flex justify-between items-end mb-2 pr-1 min-w-0">
             <label className="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider truncate">Time In</label>
             <button type="button" onClick={() => setTimeIn(getCurrentTime())} className="text-[10px] font-bold text-cyan-600 dark:text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded flex items-center gap-1 flex-shrink-0"><Clock className="w-3 h-3"/> Now</button>
          </div>
        ...
        <div className="w-full min-w-0">
          <div className="flex justify-between items-end mb-2 pr-1 min-w-0">
             <label className="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider truncate">Time Out</label>
             <button type="button" onClick={() => setTimeOut(getCurrentTime())} className="text-[10px] font-bold text-cyan-600 dark:text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded flex items-center gap-1 flex-shrink-0"><Clock className="w-3 h-3"/> Now</button>
          </div>
          <input type="time" required value={timeOut} onChange={(e) => setTimeOut(e.target.value)} 
            className="w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-cyan-500 transition-all min-w-0" />
        </div>
        <div className="xl:col-span-2 flex flex-col sm:flex-row gap-4 sm:gap-5 items-end w-full min-w-0">
          <div className="flex-1 w-full min-w-0">
            ...
            <input type="text" placeholder="E.g., UI design, bug fixing..." value={remarks} onChange={(e) => setRemarks(e.target.value)} 
              className="w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-cyan-500 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600 min-w-0" />
          </div>
          <button type="submit" disabled={loading} 
            className={`w-full sm:w-auto font-bold px-6 sm:px-8 py-3.5 rounded-xl transition-all active:scale-95 h-[48px] whitespace-nowrap shadow-lg flex-shrink-0 ${successStatus ? 'bg-emerald-500 text-white shadow-emerald-500/20' : 'bg-slate-900 dark:bg-cyan-500 hover:bg-cyan-600 dark:hover:bg-cyan-400 text-white dark:text-slate-950 shadow-slate-900/10'}`}>
